...
3. 手动修改大纲
4. 将修订后的大纲重新同步回记忆库
5. 通过 CLI 查看和修补人物、世界观记忆
6. 基于大纲 + 长期记忆 + 最近摘要写单章或一段连续章节
7. 当数据库需要修复时，可根据本地大纲和章节文件彻底重建记忆库
8. 将已生成章节导出为 Markdown 或 EPUB
...
- 从 `start_chapter` 连续写到 `end_chapter`
- 自动注入节奏控制提示，把同一段剧情要求拆分到多章中推进
- 每次重生成前自动清理当前章及之后的旧摘要
- 如果生成或记忆提取失败，会交互式询问是否重试当前章
...
该命令会先清空现有记忆表，再重新导入 `outline.txt`，然后把 `chapters/` 目录下的本地章节重新逐章回放进 SQLite。

当 `memory.db` 过期、损坏，或与本地大纲/章节文件不一致时，可以使用它修复状态。
